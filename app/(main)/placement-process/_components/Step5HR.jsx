...
export default function Step5HR({ process, setProcess }) {
  ...
  const handleInterviewComplete = async (interviewResult, error) => {
      ...
      try {
          ...
          if (!updatedProcess.hrResult.passed) {
              toast.error("Failed HR Interview.");
          } else {
              toast.success("HR Interview passed! Congratulations!");
          }
  ...
  if (evaluating) {
      return (
          <div className="flex flex-col items-center justify-center p-12 bg-card rounded-xl shadow-lg border-primary/20">
              ...
              <h2 className="text-xl font-semibold">Finalizing HR Analysis...</h2>
  ...
  return (
    <div className="w-full space-y-6">
      <div className="bg-muted/10 p-6 rounded-xl border border-primary/20 flex flex-col items-center text-center">
         ...
         <h2 className="text-3xl text-primary font-bold">Round 5: HR Interview</h2>
         <p className="text-muted-foreground mt-2 text-md max-w-xl mx-auto">
            You will now have a LiveKit real-time voice and video interview focusing on behavioral questions, culture fit, and compensation.
         </p>
         {!started && (
            <button 
